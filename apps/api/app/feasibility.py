...
from app.models import PackageDescriptor, Window, JobStatus
...
def calculate_voucher_gap(
    ...
    return max(0.0, gap)  # Gap cannot be negative (if vouchers exceed requirement, gap is 0)


def check_milestone_sanity(
    packages: List[PackageDescriptor]
) -> dict:
    """
    ...
    return {
        "first_output_ok": first_output_ok,
        "size_band_ok": size_band_ok
    }
